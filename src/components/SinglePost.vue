...
export default {
    ...
    //retrieve posts
    axios.get(`https://news.wisc.edu/wp-json/wp/v2/posts/?_embed&slug=${this.$route.params.id}`)
    .then(response => {
  ...
  methods: {
    ...
    setHeaderImage: function () {
        ...
        if (post._embedded['wp:featuredmedia'] && post._embedded['wp:featuredmedia'][0].media_details && post._embedded['wp:featuredmedia'][0].media_details.sizes){
            ...
            var imgsrc = post._embedded['wp:featuredmedia'][0].media_details.sizes.full.source_url;   
            var previmgsrc = post._embedded['wp:featuredmedia'][0].media_details.sizes.medium.source_url; 
